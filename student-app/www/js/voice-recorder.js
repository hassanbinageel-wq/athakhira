  ...
  async function saveRecording(chapterId, lessonId, questionIdx, blob) {
    return new Promise((resolve) => {
      ...
      reader.onloadend = () => {
        ...
        // فحص الحجم قبل الحفظ (حد localStorage ~5-10MB)
        const sizeKB = Math.round(dataUrl.length / 1024);
        try {
          localStorage.setItem(key, dataUrl);
          resolve({ ok: true, sizeKB });
        } catch (e) {
          ...
          resolve({
            ...
            message: isQuota
              ? `📦 مساحة التخزين ممتلئة (التسجيل ${sizeKB} كيلوبايت)\n\nاحذف بعض التسجيلات القديمة من قسم "🎧 التسجيلات" ثم حاول مجدّداً.`
              : `تعذّر حفظ التسجيل: ${e.message || 'خطأ غير معروف'}`
          });
        }
      };
      reader.onerror = () => resolve({ ok: false, reason: 'read', message: 'تعذّرت قراءة التسجيل' });
